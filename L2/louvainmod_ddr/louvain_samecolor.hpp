...
void SameColor_GetV(bool use_push_flag, ap_uint<FLAGW> *flag, int coloradj1,
                    int coloradj2, ap_uint<COLORWIDTH> *colorInx,
...
#endif
#ifdef _DEBUG_GetV
        printf("Get_pushV push_flag=%d: k=%d\t v=%d\t adj1=%d\t adj1=%d\t "
               "dgr=%d\t \n",
...
void SameColor_GetFlag2(hls::stream<int> &str_pushV, ap_uint<CSRWIDTH> *offset,
                        ap_uint<CSRWIDTH> *index, hls::stream<int> &str_flag) {
  int pushV = 0;
  DF_D_T d, e_dgr;
  e_dgr = 0, d = 0;
...
template <int CSRWIDTH>
void SameColor_GetFlag3(hls::stream<int> &str_pushV, ap_uint<CSRWIDTH> *offset,
...
template <int CSRWIDTH>
void SameColor_GetFlag4(bool wirte_push_flag, hls::stream<ap_uint<96>> &str_od,
                        ap_uint<CSRWIDTH> *index, hls::stream<int> &str_flag) {
  int pushV = 0;
  DF_D_T d, e_dgr;
  e_dgr = 0, d = 0;
...
template <int CSRWIDTH>
void SameColor_GetE(ap_uint<CSRWIDTH> *index, ap_uint<CSRWIDTH> *weight,
                    hls::stream<ap_uint<96>> &str_GetVout,
...
///////////////////////////////////////////////////////////////////////////////////////////
template <int DWIDTH>
void SameColor_GetBest_Update_Gain_bus3(
    short scl, hls::stream<ap_uint<128>> &str_Aggout, DF_W_T constant_recip,
    ap_uint<DWIDTH> *totPrev, ap_uint<DWIDTH> *commWeight,
    // output
...
#ifndef __SYNTHESIS__
#ifdef _DEBUG_GAIN
        printf("GAIN: v=%d\t, vCid=%d\t, degree=%d\t, numComm=%d\t, ki=%f\t, "
               "selfloop=%f\t \n",
               v, vCid, degree, numComm, ki, selfloop);
#endif
#endif
      }
      vcnki.vcnk.get(dout);
...
template <int DWIDTH>
void SameColor_GetBest_Update_Gain_step2(
    ap_uint<DWIDTH> *cidCcommWeighturr,
    hls::stream<StrmBus_S> &str_GainUpdate) {
  SameColor_GetBest_Update_CID_step2(cidCcommWeighturr, str_GainUpdate);
}
template <int DWIDTH>
void SameColor_GetBest_Update_Gain_step1(
    short scl, hls::stream<ap_uint<128>> &str_Aggout, DF_W_T constant_recip,
...
template <int DWIDTH>
void SameColor_GetBest_Update_Gain_step_II1(
    short scl, hls::stream<ap_uint<128>> &str_Aggout, DF_W_T constant_recip,
    ap_uint<DWIDTH> *totPrev,
    // output
...
template <int DWIDTH>
void SameColor_GetBest_Update_TOT_Csize_hash3(
    short scl, int &moves, ap_uint<DWIDTH> *cUpdateSize,
    ap_uint<DWIDTH> *totUpdate, hls::stream<StrmBus_L> &str_Cidout) {
  DF_V_T target, vCid;
  DF_W_T ki;
  StrmBus_L ctk;
  target = 0;
...
#pragma HLS DEPENDENCE variable = cUpdateSize inter false
        ...
        float tmptot =
            axi_totUpdate.rdf(addr_target); //(float)totUpdate[addr_target];
        tmptot += ki;
        // totUpdate  [addr_target] = tmptot;
        axi_totUpdate.wr(addr_target, tmptot);

        int tmpcSize =
            axi_cUpdateSize.rdi(addr_target); // cUpdateSize[addr_target];
...
template <int DWIDTH>
void SameColor_GetBest_Update_CID_step1(
    ap_uint<DWIDTH> *cidSize, hls::stream<ap_uint<160>> &str_Gainout,
    // output
    hls::stream<StrmBus_L> &str_Cidout, hls::stream<int> &str_pushV,
...
#pragma HLS PIPELINE II = 1
    if (numCandi == 0) {
      ...
      if ((d == 0) || (curr_gain_i > best_gain_i) ||
          ((curr_gain_i == best_gain_i) && (curr_gain_i > 0) &&
           (eCid < best_comm))) {
...
#ifdef _DEBUG_CID1
      printf(
          "CID1: size_best=%d\t, s_v=%d\t, best=%d\t, vCid=%d\t target=%d \n",
          size_best, size_vCid, best_comm, vCid, target);
#endif
#endif
...
template <int DWIDTH>
void SameColor_GetBest_Update_CID_step2(
    ap_uint<DWIDTH> *cidCurr, hls::stream<StrmBus_S> &str_Cid_update) {
...
template <int DWIDTH>
void SameColor_GetBest_Update_CID_step2_org(
    ap_uint<DWIDTH> *cidCurr, hls::stream<StrmBus_S> &str_Cid_update) {
  AxiMap<int, DWIDTH> axi_cidCurr(cidCurr);
...
template <int DWIDTH>
void SameColor_GetBest_Update_TOT_Csize_hash3_new(
    short scl, int &moves, ap_uint<DWIDTH> *cUpdateSize,
    ap_uint<DWIDTH> *totUpdate, hls::stream<StrmBus_L> &str_Cidout) {
  DF_V_T target, vCid;
  DF_W_T ki;
...
#pragma HLS DEPENDENCE variable = cUpdateSize inter false
      ...
      if (target != -2) {
        if (target != vCid && target != -1) {
          ...
          off = target % NUM_UNIT;
          addr = MemAgg_core_key_T<BATCH_UPDT_LOG, BATCH_UPDT>(
              target, num_cid_small[off], mem_key[off]);
          AggRAM_base_Aggregate<DF_WI_T, BATCH_UPDT_LOG, BATCH_UPDT>(
              &mem_agg[off], addr, val, isFirstAddr[off]);
          AggRAM_base_Aggregate<DF_D_T, BATCH_UPDT_LOG, BATCH_UPDT>(
              &mem_cnt[off], addr, 1, isFirstAddr[off]);
          if (num_cid_small[off] > (BATCH_UPDT - 2))
            isOneFull = true;

          off = vCid % NUM_UNIT;
          addr = MemAgg_core_key_T<BATCH_UPDT_LOG, BATCH_UPDT>(
              vCid, num_cid_small[off], mem_key[off]);
          AggRAM_base_Aggregate<DF_WI_T, BATCH_UPDT_LOG, BATCH_UPDT>(
              &mem_agg[off], addr, -val, isFirstAddr[off]);
          AggRAM_base_Aggregate<DF_D_T, BATCH_UPDT_LOG, BATCH_UPDT>(
              &mem_cnt[off], addr, -1, isFirstAddr[off]);
...
#pragma HLS PIPELINE off
        ...
        AggBatch2Mem<DWIDTH, BATCH_UPDT, BATCH_UPDT_LOG>(
            scl, cUpdateSize, totUpdate, mem_key[off], mem_agg[off],
            mem_cnt[off], num_cid_small[off], isFirstAddr[off]);
      }
      isMoved = false;
      isOneFull = false;
    }
...
void SameColor_dataflow(int numVertex, int coloradj1, int coloradj2, int &moves,
                        short scl, long long total_w_i, bool use_push_flag,
                        bool wirte_push_flag, DF_W_T &constant_recip,
                        ap_uint<DWIDTH> *offset, ap_uint<DWIDTH> *index,
                        ap_uint<DWIDTH> *weight, ap_uint<COLORWIDTH> *colorInx,
                        ap_uint<DWIDTH> *cidPrev, ap_uint<DWIDTH> *cidSize,
                        ap_uint<DWIDTH> *totPrev, ap_uint<DWIDTH> *cidCurr,
                        ap_uint<DWIDTH> *cUpdateSize,
                        ap_uint<DWIDTH> *totUpdate, ap_uint<DWIDTH> *commWeight,
                        ap_uint<CSRWIDTH> *offsetDup,
                        ap_uint<CSRWIDTH> *indexDup, ap_uint<8> *flag,
...
#pragma HLS STREAM variable = str_GetVout depth = 256
  SameColor_GetV<CSRWIDTH, COLORWIDTH>(
      use_push_flag, flag, coloradj1, coloradj2, colorInx, offset, str_GetVout);
...
#pragma HLS STREAM variable = str_Gainout_update depth = 256
  // SameColor_GetBest_Update_Gain_bus3<DWIDTH>(scl, str_Aggout, constant_recip,
  // totPrev, commWeight, str_Gainout);
  SameColor_GetBest_Update_Gain_step_II1<DWIDTH>(
      scl, str_Aggout, constant_recip, totPrev, str_Gainout_update,
      str_Gainout);
...
#pragma HLS STREAM variable = str_pushV depth = 256
  SameColor_GetBest_Update_CID_step1<DWIDTH>(cidSize, str_Gainout, str_Cidout,
                                             str_pushV, str_Cid_update);
  SameColor_GetBest_Update_CID_step2<DWIDTH>(cidCurr, str_Cid_update);
  // SameColor_GetBest_Update_TOT_Csize_org<DWIDTH>(
  SameColor_GetBest_Update_TOT_Csize_hash3_new<DWIDTH>(scl, moves, cUpdateSize,
                                                       totUpdate, str_Cidout);
