...
template <int N_E> ap_uint<32> loopNumGen(ap_uint<32> num) {
#pragma HLS INLINE
...
template <int DWIDTH>
void burstLoad(ap_uint<32> num, ap_uint<DWIDTH> *data,
               hls::stream<ap_uint<DWIDTH>> &strm) {
#pragma HLS INLINE off
  ...
  std::cout << "Load num=" << num << " base=" << base
...
#ifndef __SYNTHESIS__
#ifdef DEBUG_INIT_COMM
    std::cout << std::hex << "Loading addr=" << addr << " data=" << in
              << std::endl;
#endif
#endif
  }
...
template <int DWIDTH>
void burstWrite(ap_uint<32> num, hls::stream<ap_uint<DWIDTH>> &strm,
                ap_uint<DWIDTH> *data) {
#pragma HLS INLINE off
...
#ifndef __SYNTHESIS__
#ifdef DEBUG_INIT_COMM
  std::cout << std::hex << "Write num=" << num << " base=" << base
            << " fraction=" << fraction << std::endl;
#endif
#endif
...
#ifndef __SYNTHESIS__
#ifdef DEBUG_INIT_COMM
    std::cout << std::hex << "Writing addr=" << addr << " data=" << in
              << std::endl;
#endif
#endif
  }
...
template <int DWIDTH>
void transformOffset(int numVertex, hls::stream<ap_uint<DWIDTH>> &offset_in,
                     hls::stream<ap_uint<32>> &offset_out) {
#pragma HLS INLINE off
...
template <int DWIDTH>
void transformWeight(int numEdge, hls::stream<ap_uint<DWIDTH>> &weight_in,
...
void calculateTot(int numVertex, int numEdge, hls::stream<ap_uint<32>> &offset,
                  hls::stream<float> &weight, hls::stream<float> &totPrev,
...
#ifndef __SYNTHESIS__
#ifdef DEBUG_INIT_COMM
      std::cout << "offset=" << adj2 << " current=" << adj1
                << " totPrev=" << (float)ToDouble(kiTmp_i, scl) << std::endl;
#endif
#endif
...
#ifndef __SYNTHESIS__
#ifdef DEBUG_INIT_COMM
      std::cout << "weight=" << weight_tmp << " total=" << total_weight
                << std::endl;
#endif
#endif
    }
...
template <typename DWEIGHT, int DWIDTH, int MAXVERTEX, int MAXEDGE>
void initTot(int numVertex, hls::stream<float> &totPrev_strm,
...
template <typename DWEIGHT, int DWIDTH, int MAXVERTEX, int MAXEDGE>
void initCid(int numVertex, ap_uint<DWIDTH> cidPrev[MAXVERTEX],
...
#pragma HLS INLINE off
  ...
  const int N_E = DWIDTH / 32;
  const int loop_size =
      (numVertex % N_E) == 0 ? numVertex / N_E : numVertex / N_E + 1;
...
template <typename DWEIGHT, int DWIDTH, int MAXVERTEX, int MAXEDGE>
void initComm(int64_t numVertex, int64_t numEdge,
              ap_uint<DWIDTH> offset[MAXVERTEX],
              ap_uint<DWIDTH> weight[MAXEDGE],
              ap_uint<DWIDTH> cidPrev[MAXVERTEX],
              ap_uint<DWIDTH> cidCurr[MAXVERTEX],
              ap_uint<DWIDTH> cidSize[MAXVERTEX],
              ap_uint<DWIDTH> totPrev[MAXVERTEX], DWEIGHT &constant_recip) {
#pragma HLS INLINE off
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable = totPrev_strm depth = 32
  ...
  initCid<DWEIGHT, DWIDTH, MAXVERTEX, MAXEDGE>(numVertex, cidPrev, cidCurr,
                                               cidSize);
  ...
  calculateTot<DWEIGHT, DWIDTH, MAXVERTEX, MAXEDGE>(
      numVertex, numEdge, offset_strm1, weight_strm1, totPrev_strm,
      constant_recip);

  initTot<DWEIGHT, DWIDTH, MAXVERTEX, MAXEDGE>(numVertex, totPrev_strm,
                                               totPrev);
#ifndef __SYNTHESIS__
...
void update_color_ptr0(int numVertex,
                       hls::stream<ap_uint<COLORWIDTH>> &color_strm,
                       int colorPtr[MAXCOLOR]) {
#pragma HLS INLINE off

  int idx_tmp[8] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff,
...
template <int COLORWIDTH, int MAXVERTEX, int MAXCOLOR>
void init_color_size(int numVertex, ap_uint<COLORWIDTH> color[MAXVERTEX],
                     int colorPtr[MAXCOLOR]) {
#pragma HLS INLINE off
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable = color_strm depth = 64
  ...
  update_color_ptr0<COLORWIDTH, MAXVERTEX, MAXCOLOR>(numVertex, color_strm,
                                                     colorPtr);
}
...
void update_color_ptr1(int numVertex,
                       hls::stream<ap_uint<COLORWIDTH>> &color_strm,
                       int colorPtr[MAXCOLOR], hls::stream<int> &addr_strm) {
#pragma HLS INLINE off
...
void update_color_inx(int numVertex, hls::stream<int> &addr_strm,
...
template <int COLORWIDTH, int MAXVERTEX, int MAXCOLOR>
void remap_vertex(int numVertex, ap_uint<COLORWIDTH> color[MAXVERTEX],
                  ap_uint<COLORWIDTH> colorInx[MAXVERTEX],
                  int colorPtr[MAXCOLOR]) {
#pragma HLS INLINE off
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable = addr_strm depth = 16
  ...
  update_color_ptr1<COLORWIDTH, MAXVERTEX, MAXCOLOR>(numVertex, color_strm,
                                                     colorPtr, addr_strm);
...
#pragma HLS UNROLL
      cidSizeCurr((j + 1) * NvBt - 1, j * NvBt) =
          cidSizePrev((j + 1) * NvBt - 1, j * NvBt) +
          cidSizeUpdate((j + 1) * NvBt - 1, j * NvBt);
    }
...
void updateSameColor(int numVertex, ap_uint<DWIDTH> cidPrev[MAXVERTEX],
                     ap_uint<DWIDTH> cidCurr[MAXVERTEX],
...
#pragma HLS STREAM variable = totCurr_strm depth = 64
  ...
  // update
  calculate_tot_cidSize<DWIDTH>(numVertex, cidSizePrev_strm, totPrev_strm,
                                cidSizeUpdate_strm, totUpdate_strm,
...
void calcModularity(ap_uint<32> &loop_cnt, int numVertex, DWEIGHT &Q_prev,
                    DWEIGHT &Q_curr, DWEIGHT &deltaQ, DWEIGHT constant_recip,
                    ap_uint<DWIDTH> totPrev[MAXVERTEX],
                    ap_uint<DWIDTH> totCurr[MAXVERTEX],
...
#pragma HLS UNROLL
      ...
      std::cout << "j=" << j << " tmp1=" << (int)tmp1 << " eTot=" << eTot.f
                << " tmp2=" << (int)tmp2 << " eIn=" << eIn.f
                << " totTmp=" << totTmp_i[j] << " inTmp=" << inTmp_i[j]
                << std::endl;
#endif
#if DEBUG_CALCMODULARITY
      std::cout << "i=" << (i * NV + j) << " tmp1=" << ToDouble(tmp1, scl_sqr)
                << " tmp2=" << ToDouble(tmp2, scl) << std::endl;
#endif
#endif
    }
  }
...
Fraction:
  for (int j = 0; j < NV; j++) {
    ...
    totalIn_i += inTmp_i[j];
    //#if DEBUG_CALCMODULARITY
    //            std::cout << "i=" << j << " tmp1=" << ToDouble(totTmp_i[j],
    //            scl_sqr) << " tmp2=" <<
  ...
  Q_curr = ToDouble(totalIn_i, scl) * constant_recip -
           ToDouble(totalTot_i, scl_sqr) * constant_recip * constant_recip;
  deltaQ = Q_curr - Q_prev;

#ifndef __SYNTHESIS__
  std::cout << "totalIn=" << ToDouble(totalIn_i, scl)
            << " totalTot=" << ToDouble(totalTot_i, scl_sqr)
            << " Q_prev=" << Q_prev << " Q_curr=" << Q_curr
            << " deltaQ=" << deltaQ << std::endl;
#endif
}

template <int DWIDTH, int MAXVERTEX>
void initSameColor(int c, int numVertex,
                   ap_uint<DWIDTH> cidSizeUpdate[MAXVERTEX],
                   ap_uint<DWIDTH> totUpdate[MAXVERTEX],
                   ap_uint<DWIDTH> commWeight[MAXVERTEX],
...
template <typename DWEIGHT, int DWIDTH, int CSRWIDTH, int COLORWIDTH,
          int MAXVERTEX, int MAXDEGREE>
void sameColorWrapper(int numVertex, int numColor, int &moves,
                      long long total_w_i, short scalor,
                      DWEIGHT &constant_recip, int *colorPtr,
                      ap_uint<32> &loop_cnt, bool use_push_flag,
                      bool write_push_flag, ap_uint<CSRWIDTH> *offset,
                      ap_uint<CSRWIDTH> *index, ap_uint<CSRWIDTH> *weight,
                      ap_uint<COLORWIDTH> *colorInx, ap_uint<DWIDTH> *cidPrev,
                      ap_uint<DWIDTH> *cidSizePrev, ap_uint<DWIDTH> *totPrev,
                      ap_uint<DWIDTH> *cidCurr, ap_uint<DWIDTH> *cidSizeCurr,
                      ap_uint<DWIDTH> *totCurr, ap_uint<DWIDTH> *cidSizeUpdate,
                      ap_uint<DWIDTH> *totUpdate, ap_uint<DWIDTH> *commWeight,
                      ap_uint<CSRWIDTH> *offsetDup, ap_uint<CSRWIDTH> *indexDup,
                      ap_uint<8> *flag, ap_uint<8> *flagUpdate) {
#pragma HLS INLINE off

COLOR:
  for (int c = 0; c < numColor; c++) {
    initSameColor<DWIDTH, MAXVERTEX>(c, numVertex, cidSizeUpdate, totUpdate,
                                     commWeight, flagUpdate); //, flagUpdate
    ...
    if (loop_cnt[0] == 0) {
      ...
      SameColor_dataflow<DWIDTH, CSRWIDTH, COLORWIDTH, MAXDEGREE>(
          numVertex, coloradj1, coloradj2, moves, scalor, total_w_i,
      ...
      updateSameColor<DWIDTH, MAXVERTEX>(
          numVertex, cidPrev, cidCurr, cidSizePrev, cidSizeUpdate, cidSizeCurr,
      ...
      updateSameColor<DWIDTH, MAXVERTEX>(
          numVertex, cidPrev, cidCurr, cidSizeCurr, cidSizeUpdate, cidSizePrev,
...
template <typename DWEIGHT, int DWIDTH, int CSRWIDTH, int COLORWIDTH,
          int MAXVERTEX, int MAXEDGE, int MAXDEGREE, int MAXCOLOR>
void louvainWithColoring(
    int64_t numVertex, int64_t numColor, int64_t totItr, DWEIGHT epsilon,
    DWEIGHT &constant_recip, ap_uint<CSRWIDTH> offset[MAXVERTEX],
    ap_uint<CSRWIDTH> index[MAXEDGE], ap_uint<CSRWIDTH> weight[MAXEDGE],
    ap_uint<COLORWIDTH> color[MAXVERTEX],
    ap_uint<COLORWIDTH> colorInx[MAXVERTEX], ap_uint<DWIDTH> cidPrev[MAXVERTEX],
    ap_uint<DWIDTH> cidSizePrev[MAXVERTEX], ap_uint<DWIDTH> totPrev[MAXVERTEX],
    ap_uint<DWIDTH> cidCurr[MAXVERTEX], ap_uint<DWIDTH> cidSizeCurr[MAXVERTEX],
    ap_uint<DWIDTH> totCurr[MAXVERTEX],
    ap_uint<DWIDTH> cidSizeUpdate[MAXVERTEX],
    ap_uint<DWIDTH> totUpdate[MAXVERTEX], ap_uint<DWIDTH> commWeight[MAXVERTEX],
    ap_uint<CSRWIDTH> offsetDup[MAXVERTEX], ap_uint<CSRWIDTH> indexDup[MAXEDGE],
    ap_uint<8> flag[MAXVERTEX], ap_uint<8> flagUpdate[MAXVERTEX],
    int64_t &iteration, DWEIGHT &modularity) {
#pragma HLS INLINE off
...
#pragma HLS PIPELINE II = 3
  ...
  remap_vertex<COLORWIDTH, MAXVERTEX, MAXCOLOR>(numVertex, color, colorInx,
                                                colorPtr);
  ...
  do {
    ...
    if (tmpItrop[0] == 0) {
      sameColorWrapper<DWEIGHT, DWIDTH, CSRWIDTH, COLORWIDTH, MAXVERTEX,
                       MAXDEGREE>(
          numVertex, numColor, moves, total_w_i, scalor, constant_recip,
          colorPtr, loop_cnt, config_push_flag, wirte_push_flag, offset, index,
          weight, colorInx, cidPrev, cidSizePrev, totPrev, cidCurr, cidSizeCurr,
          totCurr, cidSizeUpdate, totUpdate, commWeight, offsetDup, indexDup,
          flag, flagUpdate);
    } else {
      sameColorWrapper<DWEIGHT, DWIDTH, CSRWIDTH, COLORWIDTH, MAXVERTEX,
                       MAXDEGREE>(
...
#endif
    calcModularity<DWEIGHT, DWIDTH, MAXVERTEX>(loop_cnt, numVertex, Q_prev,
                                               Q_curr, deltaQ, constant_recip,
                                               totPrev, totCurr, commWeight);
 ...
 *
 * @param config0 table for the int64 parameter of louvain, numVertex, numColor,
 * total iteration times and so on.
 * @param config1 table for the double parameter of louvain, epsilon,
 * constant_recip, modularity and so on.
 ...
 */
inline void
kernelLouvainTop(int64_t *config0, DWEIGHT *config1,
                 ap_uint<CSRWIDTHS> *offsets, ap_uint<CSRWIDTHS> *indices,
                 ap_uint<CSRWIDTHS> *weights, ap_uint<COLORWIDTHS> *colorAxi,
                 ap_uint<COLORWIDTHS> *colorInx, ap_uint<DWIDTHS> *cidPrev,
                 ap_uint<DWIDTHS> *cidSizePrev, ap_uint<DWIDTHS> *totPrev,
                 ap_uint<DWIDTHS> *cidCurr, ap_uint<DWIDTHS> *cidSizeCurr,
                 ap_uint<DWIDTHS> *totCurr, ap_uint<DWIDTHS> *cidSizeUpdate,
                 ap_uint<DWIDTHS> *totUpdate, ap_uint<DWIDTHS> *cWeight,
                 ap_uint<CSRWIDTHS> *offsetsDup, ap_uint<CSRWIDTHS> *indicesDup,
                 ap_uint<8> *flag, ap_uint<8> *flagUpdate) {
  // clang-format off
    DWEIGHT constant_recip = 0;

  // clang-format on
  xf::graph::initComm<DWEIGHT, DWIDTHS, VERTEXS, EDGES>(
      config0[0], config0[3], offsets, weights, cidPrev, cidCurr, cidSizePrev,
      totPrev, constant_recip);

  xf::graph::louvainWithColoring<DWEIGHT, DWIDTHS, CSRWIDTHS, COLORWIDTHS,
                                 VERTEXS, EDGES, DEGREES, COLORS>(
      config0[0], config0[1], 0, config1[0], constant_recip, offsets, indices,
      weights, colorAxi, colorInx, cidPrev, cidSizePrev, totPrev, cidCurr,
      cidSizeCurr, totCurr, cidSizeUpdate, totUpdate, cWeight, offsetsDup,
      indicesDup, flag, flagUpdate, config0[2], config1[1]);

  xf::graph::renumberClusters_ghost<DWIDTHS, VERTEXS>(
      config0[0], config0[4], config0[5], cidPrev, cidSizePrev);
}
