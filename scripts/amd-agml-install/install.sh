...
supported_products="agml cosinesim louvainmod mis fuzzymatch"
...
if [[ $device_supported -eq 1 && $product_supported -eq 1 ]] ; then
    echo "INFO: Installing AMD $product product for $device and its dependencies on $OSDIST $OSREL_VER..."
fi
...
if [[ $OSDIST == "ubuntu" ]]; then
    ...
    if [[ $product == "cosinesim" ]]; then
        ...
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD CosineSim. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo apt install --reinstall $pkg_dir/cosinesim/amd-cosinesim*.deb
    elif [[ $product == "louvainmod" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD LouvainMod. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo apt install --reinstall $pkg_dir/louvainmod/amd-louvainmod*.deb
    elif [[ $product == "mis" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD MIS. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo apt install --reinstall $pkg_dir/mis/amd-mis*.deb
    elif [[ $product == "fuzzymatch" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD FuzzyMatch. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo apt install --reinstall $pkg_dir/fuzzymatch/amd-fuzzymatch*.deb
    elif [[ $product == "agml" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD AGML. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo apt install --reinstall $pkg_dir/agml/amd-agml*.deb
    fi
fi
...
if [[ $OSDIST == "centos" ]]; then
    ...
    if [[ $product == "cosinesim" ]]; then
        ...
        printf "\nINFO: Install AMD CosineSim package\n"
        sudo yum install $pkg_dir/cosinesim/amd-cosinesim*.rpm -y

        printf "\nINFO: Install AMD Recommend Engine package\n"
        sudo yum install $pkg_dir/recomengine/amd-recomengine*.rpm -y
    elif [[ $product == "fuzzymatch" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD FuzzyMatch. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo yum install $pkg_dir/fuzzymatch/amd-fuzzymatch*.rpm -y
    elif [[ $product == "agml" ]]; then
        printf "\n-------------------------------------------------------------\n"
        printf "INFO: Install AMD AGML. Enter sudo password if asked."
        printf "\n-------------------------------------------------------------\n"
        sudo yum install $pkg_dir/agml/amd-agml*.rpm -y
    fi    
fi

# copy requirements.txt to /opt/amd/apps/graphanalytics/
sudo cp $SCRIPTPATH/requirements.txt /opt/amd/apps/agml/
