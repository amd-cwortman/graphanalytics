...
# Source directory for Log Similarity demo UDFs
...
if [ $uninstall -eq 1 ]; then
    echo "INFO: Uninstalling Log Similarity demo UDFs from node $HOSTNAME"
    if [ -f $tg_udf_dir/ExprFunctions.hpp ] \
            && [ $(grep -c 'mergeHeaders.*logSimilarityDemo' $tg_udf_dir/ExprFunctions.hpp) -gt 0 ]
    then
        if [ -x $tg_udf_dir/mergeHeaders.py ]; then
            mv $tg_udf_dir/ExprFunctions.hpp $tg_udf_dir/ExprFunctions.hpp.prev
            python3 $tg_udf_dir/mergeHeaders.py -u $tg_udf_dir/ExprFunctions.hpp.prev logSimilarityDemo \
    ...
    echo "INFO: Uninstalling Log Similarity demo auxiliary files"
    rm -f $tg_udf_dir/codevector.hpp
    rm -f $tg_udf_dir/logSimilarityDemo.hpp
...
#
# Check that the Recommendation Engine is installed first, as the Log Similarity demo depends on it
#
...
#
# Check whether the Log Similarity demo UDF sources are newer than their installed counterparts.
...
if [ $force_clean -eq 1 ]; then
    ...
    if [ ! $plugin_src_dir/logSimilarityDemo.hpp -nt $tg_udf_dir/logSimilarityDemo.hpp ] \
        && [ ! $plugin_src_dir/codevector.hpp -nt $tg_udf_dir/codevector.hpp ]; then
        echo "INFO: Log Similarity demo UDFs are up to date.  Skipping installation."
        exit 0
    fi
fi

#
# Install Log Similarity demo UDFs to ExprFunctions.hpp
#

echo "INFO: Installing Log Similarity demo UDFs to node $HOSTNAME"
mv $tg_udf_dir/ExprFunctions.hpp $tg_udf_dir/ExprFunctions.hpp.prev
python3 $tg_udf_dir/mergeHeaders.py $tg_udf_dir/ExprFunctions.hpp.prev $plugin_src_dir/logSimilarityDemo.hpp \
     > $tg_udf_dir/ExprFunctions.hpp

#
# Install other Log Similarity demo headers to TigerGraph installation
#

echo "INFO: Installing Log Similarity demo auxiliary files"
cp -f $plugin_src_dir/codevector.hpp $tg_udf_dir
cp -f $plugin_src_dir/logSimilarityDemo.hpp $tg_udf_dir
